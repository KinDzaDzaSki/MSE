...
function logoHTML(sym, name, site, size = 22, fav = false, favv = 0) {
  if (window.W && W.companyIcon) return W.companyIcon(sym, name, site, size, fav, favv);
  return '';
}
...
function renderDivTable() {
  ...
  body.innerHTML = rows.map((r) => {
    ...
    return `<tr data-sym="${esc(r.symbol)}">
      <td class="sym"><div class="sym-inner">${starBtnHTML(r.symbol)}${logoHTML(r.symbol, r.name, r.site, 22, r.fav, r.favv)}<span class="sym-text">${esc(r.symbol)}</span></div></td>
  ...
  try {
    ...
    content.innerHTML = `
      <div class="company-head">
        <h2>${logoHTML(symbol, q.name, q.site, 30, q.fav, q.favv)}${esc(symbol)}</h2>
        ${starBtnHTML(symbol)}
