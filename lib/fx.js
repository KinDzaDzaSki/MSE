...
async function fetchFX() {
  ...
  // Full list for the /kursna-lista page + the FX popover (same fetch).
  const all = latest.ExchangeRates.map((r) => ({
    code: r.Oznaka,
    name: r.Naziv,
    nameEn: r.NazivEn,
    unit: parseFloat(r.Nomin) || 1,
    buy: r.Kupoven != null && r.Kupoven !== '' ? parseFloat(r.Kupoven) : null,
    mid: r.Sreden != null && r.Sreden !== '' ? parseFloat(r.Sreden) : null,
    sell: r.Prodazen != null && r.Prodazen !== '' ? parseFloat(r.Prodazen) : null,
  })).filter((r) => r.mid != null);
  return {
...
const MISSING_LIST_RETRY = 10 * 60 * 1000; // if the full list is absent, retry sooner

async function maybeRefreshFX() {
  try {
    const [listDate, checkedAt, cachedList] = await Promise.all([
      db.getMeta('fx_date', null),
      db.getMeta('fx_checked_at', 0),
      db.getMeta('fx_all', null),
    ]);
    const checked = typeof checkedAt === 'number' ? checkedAt : 0;
    const hasList = Array.isArray(cachedList) && cachedList.length > 0;
    if (listDate === todayKey() && hasList) return; // today's list already cached
    // The full list can be missing on installs created before it was cached —
    // retry those sooner than the normal daily guard.
    if (Date.now() - checked < (hasList ? CHECK_INTERVAL : MISSING_LIST_RETRY)) return;
    ...
    await db.setMeta('fx_all', fx.all || []);
    log.info(`FX ready: EUR ${fx.eur} USD ${fx.usd} (${fx.date}) — ${(fx.all || []).length} currencies cached`);
  } catch (e) {
...
// Full NBRM list (buy/mid/sell) for the cached date.
async function getFXList() {
  const [list, date] = await Promise.all([
    db.getMeta('fx_all', []),
    db.getMeta('fx_date', null),
  ]);
  return { list: Array.isArray(list) ? list : [], date };
}

module.exports = { fetchFX, maybeRefreshFX, getFX, getFXList };
