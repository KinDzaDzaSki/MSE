...
async function migrate() {
  ...
  `);
  // These two tables were added after the RLS rollout — keep the invariant:
  // movers is public read, push_subscriptions is server-only (it holds push
  // endpoint + keys). The app connects as the table owner and bypasses RLS
  // (as it already does for the other tables). Best-effort: a role without
  // ownership must not break boot.
  try {
    await query('ALTER TABLE movers ENABLE ROW LEVEL SECURITY');
    await query('ALTER TABLE push_subscriptions ENABLE ROW LEVEL SECURITY');
    await query('DROP POLICY IF EXISTS "anon read movers" ON movers');
    await query('CREATE POLICY "anon read movers" ON movers FOR SELECT TO anon, authenticated USING (true)');
  } catch (e) {
    log.warn(`RLS setup for movers/push_subscriptions skipped: ${e.message}`);
  }
