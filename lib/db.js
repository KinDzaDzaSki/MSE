...
async function getAllFavicons() {
  if (USE_MEMORY) return Object.fromEntries(mem.favicons);
  const r = await query('SELECT symbol, data, content_type, fetched_at FROM favicons');
  const out = {};
  for (const row of r.rows) out[row.symbol] = { data: row.data, type: row.content_type, fetchedAt: Number(row.fetched_at) };
  return out;
}
