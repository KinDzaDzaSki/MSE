...
let state = {
  ...
  lastPoll: null,
  polling: false,
  // symbol -> downsampled 1Y closes, rebuilt at the end of every poll and
  // persisted to meta so clients can draw sparklines straight from /api/quotes.
  sparks: {},
};
...
async function init() {
  ...
  // Sparkline series (downsampled 1Y closes) are persisted so a cold serverless
  // start can serve them without re-reading the whole history table.
  try {
    const sparks = await db.getMeta('sparks', null);
    if (sparks && typeof sparks === 'object') {
      state.sparks = sparks;
      log.info(`spark series loaded: ${Object.keys(sparks).length}`);
    }
  } catch (e) {
...
// Downsample a closes series to ~n points (keeps first + last so the shape and
// the endpoints stay honest). Returns [] when there is nothing to draw.
function downsample(values, n = 48) {
  const v = (values || []).filter((x) => x != null && !isNaN(x));
  if (v.length <= n) return v;
  const step = (v.length - 1) / (n - 1);
  const out = [];
  for (let i = 0; i < n; i++) out.push(v[Math.round(i * step)]);
  return out;
}

function getSpark(symbol) {
  return state.sparks[symbol] || null;
}
function getSparks() {
  return state.sparks;
}

// Build the spark series from stored history — pure DB reads, no MSE requests.
// Used on boot when the cached series are missing (fresh deploy, new feature),
// so the dashboard gets instant sparklines even while the market is closed.
async function rebuildSparksFromDb() {
  ...
  for (let i = 0; i < syms.length; i += BATCH) {
    ...
    await Promise.all(slice.map(async (sym) => {
      try {
        const rows = await db.getHistory(sym);
        const vals = rows
          .filter((r) => r.last != null)
          .slice()
          .sort((a, b) => parseMDY(a.date) - parseMDY(b.date))
          .map((r) => r.last);
        if (vals.length > 1) sparks[sym] = downsample(vals);
      } catch (e) { /* skip symbol */ }
    }));
  }
  if (Object.keys(sparks).length) {
    state.sparks = sparks;
    await db.setMeta('sparks', sparks);
    log.info(`spark series rebuilt from DB: ${Object.keys(sparks).length} symbols`);
  }
  return Object.keys(sparks).length;
}
...
async function pollQuotes() {
  ...
  try {
    ...
    // the downsampled closes here (one pass per poll) instead of making every
    // client fetch history. Persisted so cold starts serve them immediately.
    try {
      const sparks = {};
      for (const sym of Object.keys(results)) {
        const hist = (metrics[sym] && metrics[sym].hist) || [];
        const vals = hist
          .filter((r) => r.last != null)
          .slice()
          .sort((a, b) => parseMDY(a.date) - parseMDY(b.date))
    ...
    log.info(`poll ${ok}/${primaries.length} primaries (+${secCount} series) in ${Date.now() - t0}ms; ${activeSet.length} active, ${liquidCount} liquid; sparks ${Object.keys(state.sparks).length}`);
  } finally {
    state.polling = false;
  }
}
...
function startScheduler({ pollIntervalMs = 60000 } = {}) {
  (async () => {
    ...
    try {
      if (Object.keys(state.sparks).length < Math.max(5, Math.floor(getSymbols().length * 0.5))) {
