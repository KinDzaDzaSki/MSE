...
// Copy the `fav` flag onto stored quotes: `fav` = the issuer has an official
// website (so the client attempts the icon chain). Keeps quotes consistent even
// if they were written before this flag existed.
async function patchQuoteFavs() {
  const [quotes, companies] = await Promise.all([db.getAllQuotes(), db.getAllCompanies()]);
  const patch = {};
  for (const [sym, q] of Object.entries(quotes)) {
    const has = !!(companies[sym] && companies[sym].website);
    if (!!q.fav !== has) patch[sym] = { ...q, fav: has };
  }
  const n = Object.keys(patch).length;
  if (n) await db.upsertQuotes(patch);
  return n;
