const { fetchSymbolList, fetchHistory, fetchQuote, fetchIndex, fetchIndexHistory, changeFromHistory, week52ChangeFromHistory, isEquity, fetchFinancialData, sortByDate } = require('./scraper');
const db = require('./db');
...
async function pollIndices() {
  try {
    ...
    // Append today's row to the index history so the 1Y MBI10 chart keeps
    // growing. Date uses MSE's non-padded M/D/YYYY format ("9/14/2026") so
    // ON CONFLICT (symbol, date) dedups against scraped rows.
    if (mbi.value != null) {
      const now = nowSkopje();
      const date = `${now.getMonth() + 1}/${now.getDate()}/${now.getFullYear()}`;
      await db.upsertHistoryRows('MBI10', [{ date, last: mbi.value, max: mbi.value, min: mbi.value }]);
    }
