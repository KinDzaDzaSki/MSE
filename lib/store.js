...
async function backfillIndexHistory(code = 'MBI10') {
  ...
  for (const year of years) {
    try {
      ...
      console.error(`[backfillIndex] ${code} year ${year} error:`, e.message);
    }
  }
  if (all.length) {
    ...
    console.log(`[backfillIndex] ${code}: ${all.length} rows (${fresh.length} new)`);
  }
  return all;
}
...
function startScheduler({ pollIntervalMs = 60000 } = {}) {
  (async () => {
    try {
      ...
      console.error('[scheduler] initial poll error:', e.message);
    }
    try {
      await pollIndices();
    } catch (e) {
      console.error('[scheduler] initial index error:', e.message);
    }
    // One-time backfill on first install: if no history exists yet, populate it.
    try {
      const syms = getSymbols();
      if (syms.length) {
        const firstHist = await getHistory(syms[0]);
        if (firstHist.length === 0) {
          console.log('[scheduler] first run — backfilling history for all symbols...');
          for (let i = 0; i < syms.length; i++) {
            await backfillHistory(syms[i], 365);
          }
          console.log(`[scheduler] one-time backfill complete (${syms.length} symbols)`);
      ...
      console.error('[scheduler] one-time backfill error:', e.message);
    }
  })();
  setInterval(async () => {
    if (isMarketOpen()) {
