...
async function pollQuotes() {
  ...
  try {
    ...
    for (const sym of state.symbols) {
      ...
      ok++;
      // Track actively-traded companies: any equity issuer is active. The
      // history-backed hasTraded check is only a secondary guard so that an
      // equity with no trading at all (e.g. before backfill) is not dropped.
      const hasTraded = hist.some((r) => r.volume && r.volume > 0);
      if (isEquity(q.segment, q.name, sym)) activeSet.push(sym);
      else if (hasTraded) activeSet.push(sym);
