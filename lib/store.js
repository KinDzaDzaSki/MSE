...
// traded, plus that subscriber's own watchlist symbols with their change.
function buildPushPayload(lang, movers, watchRows) {
  const mk = lang !== 'en';
  const line = (label, rows) => {
    const items = (rows || []).slice(0, 3).map((r) => `${r.symbol} ${fmtPct(r.changePct)}`);
  ...
  if (watchRows && watchRows.length) {
    parts.push(`★ ${mk ? 'Твои акции' : 'Your watchlist'}: ${watchRows.slice(0, 5).map((r) => `${r.symbol} ${fmtPct(r.changePct)}`).join(', ')}`);
  }
  return {
...
async function sendMoversNotifications(movers) {
  ...
  for (const sub of subs) {
    if (sub.lastNotifiedDay === movers.asOf) continue;
    const watchRows = (sub.watchlist || [])
      .map((s) => quotes[s])
      .filter((q) => q && q.changePct != null)
    ...
    try {
      ...
      else log.warn(`push send failed (${code || e.message})`);
    }
  }
  if (sent || dropped) log.info(`movers push: ${sent} sent, ${dropped} dropped`);
}
